...
<?php
	if (isset($_GET['action']) && $_GET['action']=="detail_transaksi_tambah") {
				...
				<div class="right">
					<script type="text/javascript">
						function gotojenis(val){
							...
							window.location.href="laporanmasuk.php?jenis="+value+"";
						}
						function gotofilter(val){
							var value=val.options[val.selectedIndex].value;
							
							window.location.href="laporanmasuk.php?jenis=<?php if (isset($_GET['jenis'])) {
								echo $_GET['jenis'];
							} ?>&filter_record="+value;
						}
					</script>
					<a style="float: left;" href="cetak_barang_masuk.php" class="btnblue" target="_blank"><i class="fa fa-print"></i> Cetak SEMUA</a>
    ...
    <form action="cetak_laporan_masuk.php" style="display: inline;" target="_blank" method="post">
			...
			<tbody>
					...
					if (isset($_GET['filter_record'])) {
						...
						$root->filter_tampil_laporan_masuk($_GET['filter_record'],$aksi1);
					}else{
					$root->tampil_laporan_masuk2();
					}
					?>
			</tbody>
			</table>
