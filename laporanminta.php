...
<?php
	if (isset($_GET['action']) && $_GET['action']=="detail_transaksi_minta") {
				...
				<div class="right">
					<script type="text/javascript">
						function gotojenis(val){
							...
							window.location.href="laporanminta.php?jenis="+value+"";
						}
						function gotofilter(val){
							var value=val.options[val.selectedIndex].value;
							
							window.location.href="laporanminta.php?jenis=<?php if (isset($_GET['jenis'])) {
								echo $_GET['jenis'];
							} ?>&filter_record="+value;
						}
					</script>
					<a style="float: left;" href="cetak_barang_minta.php" class="btnblue" target="_blank"><i class="fa fa-print"></i> Cetak SEMUA</a>
					<span style="float: left;
    padding: 5px;
    margin-right: 10px;
    color: #666;">Filter dan cetak :</span>
    <form action="cetak_laporan_masuk.php" style="display: inline;" target="_blank" method="post">
					...
					<select class="leftin1" onchange="gotofilter(this)" required="required" name="tgl_laporan">
						<?php
							if (isset($_GET['jenis'])&&$_GET['jenis']=='perhari') {
								...
								<?php
								$data=$root->con->query("select distinct date(tgl_transaksi) as tgl_transaksi from transaksi_tambah order by id_transaksi desc");
						...
						<?php
							$data=$root->con->query("select distinct EXTRACT(YEAR FROM tgl_transaksi) AS OrderYear,EXTRACT(MONTH FROM tgl_transaksi) AS OrderMonth from transaksi_tambah order by id_transaksi desc");
							while ($f=$data->fetch_assoc()) {
								?>
			...
			<table class="datatable" id="datatable">
				...
				<tr>
					...
					<th width="40px">Tanggal Transaksi</th>
			...
			<tbody>
					...
					if (isset($_GET['filter_record'])) {
						...
						$root->filter_tampil_laporan_minta($_GET['filter_record'],$aksi1);
					}else{
					$root->tampil_laporan_minta2();
					}
					?>
</tbody>
