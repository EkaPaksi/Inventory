			...
			<tbody>
                ...
                ?>
                    <tr>
						...
						<td>
							<a href="?action=detail_transaksi&id_transaksi=<?= $data['id_transaksi'] ?>" class="btn bluetbl m-r-10"><span class="btn-edit-tooltip">Detail</span><i class="fa fa-eye"></i></a>
							<a href="cetak_nota.php?oid=<?= base64_encode($data['id_transaksi']) ?>&id-uid=<?= base64_encode($data['nama_pembeli']) ?>&inf=<?= base64_encode($data['no_invoice']) ?>&tb=<?= base64_encode($data['total_bayar']) ?>&uuid=<?= base64_encode(date("d-m-Y",strtotime($data['tgl_transaksi']))) ?>" target="_blank" class="btn bluetbl"><span class="btn-hapus-tooltip">Cetak</span><i class="fa fa-print"></i></a>
						</td>
