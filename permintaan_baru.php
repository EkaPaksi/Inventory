<script type="text/javascript">
	document.title="Permintaan Baru";
...
<div class="content">
	<div class="padding">
		<div class="bgwhite">
			<div class="padding">
				<h3 class="jdl">Entry Permintaan</h3>
				<form class="form-input" method="post" action="handler.php?action=tambah_tempo_minta" style="padding-top: 30px;">
					...
					<label>Ket :</label>
					<input required="required" type="text" name="ket">
					<input type="hidden" name="trx" value="<?php echo date("d")."/REQ/".$_SESSION['id']."/".date("y") ?>">
		...
		<div class="bgwhite">
			...
			<tbody id="contenth">
				<?php
				$trx=date("d")."/REQ/".$_SESSION['id']."/".date("y");
				...
				while ($f=$data->fetch_assoc()) {
					?><tr>
						...
						<td><?= $f['nama_barang'] ?></td>
						<td><?= $f['jumlah_beli'] ?></td>
						<td><a href="handler.php?action=hapus_tempo_minta&id_tempo=<?= $f['id_subtransaksi'] ?>&id_barang=<?= $f['id_barang'] ?>&jumbel=<?= $f['jumlah_beli'] ?>" class="btn redtbl"><span class="btn-hapus-tooltip">Cancel</span><i class="fa fa-close"></i></a></td>
						</tr>
					<?php
				}
				?>
			...
			<br>
			<form class="form-input" action="handler.php?action=selesai_transaksi_minta" method="post">
